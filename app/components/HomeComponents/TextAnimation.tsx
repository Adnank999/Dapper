...
interface Props extends HTMLMotionProps<"h2"> {
  ...
  delay?: number
  duration?: number
  /** Component you want appended and animated as part of the sequence */
  colorfulText?: React.ReactNode
  /** Insert a space before colorfulText (default: true) */
  spaceBeforeColorful?: boolean
  /** Set true to remove staggering & delay so text + colorful animate together */
  sync?: boolean
}
...
export const TextAnimate: FC<Props> = ({
  ...
  //   >
  //     {letters.map((letter, index) => (
  //       <motion.span key={index} variants={child} className="leading-5 text-highlight text-3xl md:text-5xl lg:text-8xl font-bold !font-my-font-bold text-center">
  //         {letter === " " ? "\u00A0" : letter}
  ...
  //      {colorfulText && (
  //       <motion.span variants={child} className="ml-2 inline-block leading-5 text-highlight text-3xl md:text-5xl lg:text-8xl font-bold !font-my-font-bold text-center">
  //         {colorfulText}
  //       </motion.span>
  //     )}
  ...
  // )
    ...
    <motion.h2
      ...
      initial="hidden"
      animate="visible"
      className="mt-10 leading-5 text-highlight text-6xl lg:text-8xl font-bold !font-my-font-bold text-center py-5 pb-8 px-8 "
      {...props}
    >
      {letters.map((letter, i) => (
        <motion.span
          key={`l-${i}`}
          variants={child}
          className="ta-char inline-block" // ⬅️ tag each character
        >
          {letter === " " ? "\u00A0" : letter}
        </motion.span>
      ))}
      ...
      {colorfulText && (
        <motion.span
          key="colorful"
          variants={child}
          className="ta-colorful inline-block ml-2 align-baseline" // ⬅️ wrap colorfulText
        >
          {props.spaceBeforeColorful ? "\u00A0" : null}
          {colorfulText}
        </motion.span>
      )}
    </motion.h2>
